...
def predict_rent_endpoint():
    ...
    try:
        ...
        bedrooms = float(data.get("bedrooms") or data.get("bhk") or 2)
        beds = float(data.get("beds") or bedrooms)
        bathrooms = float(data.get("bathrooms") or data.get("bathroom") or data.get("bathroomsAirbnb") or 1.5)
        latitude = float(data.get("latitude") or default_lat)
        longitude = float(data.get("longitude") or default_lng)
        room_type = data.get("room_type") or data.get("roomType") or "Entire home/apt"
        property_type = data.get("property_type") or data.get("propertyType") or "Entire home"
        is_superhost = int(data.get("is_superhost") or data.get("isSuperhost") or 0)
        min_nights = float(data.get("min_nights") or data.get("minimum_nights") or data.get("minNights") or data.get("minimumNights") or 2)
        max_nights = float(data.get("max_nights") or data.get("maximum_nights") or data.get("maxNights") or data.get("maximumNights") or 1125)
        avail_365 = float(data.get("avail_365") or data.get("availability_365") or data.get("avail365") or data.get("availability365") or 180)
        num_reviews = float(data.get("num_reviews") or data.get("number_of_reviews") or data.get("numReviews") or data.get("numberOfReviews") or 25)
        rating_raw = data.get("rating") or data.get("review_scores_rating") or data.get("reviewScoresRating") or 4.85
        rating = float(rating_raw)
        rating_cleanliness = float(data.get("rating_cleanliness") or data.get("ratingCleanliness") or 4.90)
        rating_location = float(data.get("rating_location") or data.get("ratingLocation") or 4.85)
        identity_verified = int(data.get("host_identity_verified") or data.get("identity_verified") or data.get("identityVerified") or 1)
        instant_bookable = int(data.get("instant_bookable") or data.get("instantBookable") or 0)
