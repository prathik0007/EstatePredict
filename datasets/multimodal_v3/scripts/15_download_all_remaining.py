...
AUDIT_CSV = os.path.join(RESULTS_DIR, "image_download_audit_full.csv")
...
def download_and_validate_single_image(row_tuple):
    ...
    try:
        r = session.get(img_url, timeout=3.5)
...
def download_full_cohort(max_workers=36):
    print("=" * 70, flush=True)
    print("STAGE 1: COMPLETE ASHEVILLE MULTIMODAL V3 IMAGE DOWNLOAD (ALL 3,110 LISTINGS)", flush=True)
    print("=" * 70, flush=True)
    
    df_raw = pd.read_csv(RAW_CSV)
    df_valid = df_raw[df_raw['price'].notna() & (df_raw['price'] != "") & df_raw['picture_url'].notna()].copy()
    
    tasks = [(row['id'], row['picture_url']) for _, row in df_valid.iterrows()]
    total = len(tasks)
    print(f"Total candidate listings in cohort: {total} | Workers: {max_workers}", flush=True)
    ...
    print(f"\nFull download audit saved to {AUDIT_CSV}", flush=True)
    
    succ_df = audit_df[audit_df['status'].isin(['success', 'success_cached'])]
    fail_df = audit_df[~audit_df['status'].isin(['success', 'success_cached'])]
    
    print("\n" + "=" * 50, flush=True)
    print("FULL COHORT IMAGE DOWNLOAD SUMMARY:", flush=True)
    print("=" * 50, flush=True)
    print(f"  - Total Candidate Listings : {total}", flush=True)
